    ...
    return (
        <div className="gap-2 hidden mt-8 lg:grid lg:grid-cols-4 mx-auto" style={lightbox ? {maxWidth: '25rem', gap: '1.5rem'} : null}>
            { images.map((img, index) => {
                return (
                    <div key={index} className="relative rounded-lg hover:border-orange hover:cursor-pointer group" onClick={() => showImage(index)} role="button" aria-label={`View product image ${index + 1}`}>
